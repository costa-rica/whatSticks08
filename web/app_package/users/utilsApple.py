...
def decompress_and_save_apple_health(apple_health_dir, apple_health_data):
    ...
    ##################################################################
    logger_users.info(f'-- START users_routes/utilsApple def decompress_and_save_apple_health')
    # logger_users.info(f'-- ***************************************')
    ...
    logger_users.info(f'- path_to_zip_file: {path_to_zip_file}')
    ...
    with zipfile.ZipFile(path_to_zip_file, 'r') as zip_ref:
        ...
        if xml_filename == 'apple_health_export/':# <--- means it comes straight from Apple Health download
            logger_users.info(f'- User submitted export.zip straight from Apple Health')
            ...
            for file in new_file_path_files_list:
                if file[-4:]=='.xml':
                    ...
                    logger_users.info(f'---> new_file_path: {new_file_path}')

        else:# <---- some other file maybe something i zipped
            logger_users.info(f'- User submitted a zipped version of the xml file')
    ...
    # logger_users.info(f'2: apple_health_dir: {apple_health_dir}')
    # logger_users.info(f'-- apple_health filename: {apple_health_data.filename}')
    # logger_users.info(f'-- apple_health filename type: {type(apple_health_data.filename)}')
    # logger_users.info(f'-- apple_health filename type: {apple_health_data.filename[:-4]}')
    ...
    #     logger_users.info(f'---> searching for new_file in apple_health_export dir')
    #     new_file_path_files_list = os.listdir(os.path.join(apple_health_dir,'apple_health_export'))
    #     for file in new_file_path_files_list:
    #         if file[-4:]=='.xml':
    #             new_file_path = os.path.join(apple_health_dir,'apple_health_export', file)
    #             logger_users.info(f'---> new_file_path: {new_file_path}')
    #     logger_users.info(f'---> from Apple Health download')
    # else:# <---- some other file maybe something i zipped
    #     new_file_path = os.path.join(apple_health_dir,'apple_health_export',xml_filename )
    #     logger_users.info(f'---> from Nick edited or other source')
    # # new_file_path = os.path.join(apple_health_dir,'export.xml' )
    # # new_file_path = os.path.join(apple_health_dir,xml_filename )
    #     logger_users.info(f'---> new_file_path (user zipped): {new_file_path}')

    # logger_users.info(f'3: apple_health_dir: {apple_health_dir}')
    ...
    logger_users.info(f'-- END users_routes/utilsApple def decompress_and_save_apple_health')
...
def add_apple_to_db(xml_dict):
    ...
    logger_users.info(f'-- in users/utilsApple.py def add_apple_to_db --')
    #######################################
...
# Columns with dictionary structures i di'tn kwon what to do with
    ...
    if len(df_existing) > 0:
        ...
        logger_users.info(f'-- length of EXISTING apple health data: {len(df_existing)}')
        logger_users.info(f'-- length of NEW apple health data: {len(df)}')
...
def clear_df_files(USER_ID):
    logger_users.info('-- in users/utilsApple.py def clear_df_files --')
